...
require 'net/ssh/session_logger'
...
module Net
  module SSH
    class Session
      ...
      # Assign a logger for the session
      # @param object [SessionLogger]
      def logger= (object)
        if !object.kind_of?(SessionLogger)
          raise ArgumentError, "Session logger required"
        end
      ...
      def run(command)
        ...
        result = SessionCommand.new(command, output, exit_code)
